...
describe("rome-angular", function() {
    ...
    describe("no config", function() {
        ...
        // Test that it worked
        it("(input with no config) should display date as YYYY-MM-DD HH:mm", function() {
            expect(input.val()).toEqual("1982-10-16 07:22");
        });
        
        it("(input with no config) should have date as its value", function() {
            expect(rome(input[0]).getDate()).toEqual(scope.date);
        });
        
        it("(div with no config) should have date as its value", function() {
    ...
    describe("with configProvider", function() {
        ...
        // Test that it worked
        it("(input with configProvider) should display date as DD/MM/YYYY HH:mm", function() {
            expect(input.val()).toEqual("16/10/1982 07:22");
        });
        
        it("(input with configProvider) should have date as its value", function() {
            expect(rome(input[0]).getDate()).toEqual(scope.date);
        });
        ...
        describe("with custom configuration", function() {
            ...
            it("(input with custom configuration) should display date as DD-MM-YYYY HH.mm", function() {
                expect(input.val()).toEqual("16-10-1982, 07.22");
            });
            
            it("(input with custom configuration) should have date as its value", function() {
                expect(rome(input[0]).getDate()).toEqual(scope.date);
            });
        });
    });
